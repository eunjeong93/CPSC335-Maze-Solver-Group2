...
# how to run: python BFS_maze.py <maze_file_path>
...
def bfs(maze, start, end):
    ...
    Returns:
        ...
        path : fianl (shortest) path from S to E
        path_length : length of the final path from S to E
        final_visited : 2D array marking the cells that are part of the final (shortest) path (For visualization)
        runtime : time taken to execute the BFS algorithm
        total_visited : 2D array marking all the cells that were visited during the BFS
    """
...
if __name__ == "__main__":
    ...
    # apply BFS algorithm 
    result, path, path_length, final_visited, runtime, total_visited = bfs(maze, start, end)
